  ...
  return (
    <main className="mb-7 px-5">
      <div className="p-8 max-w-1320 w-full mx-auto bg-white rounded-20 overflow-auto">
        ...
        <div className={`flex `+ ( riskLevelValid ? `justify-center` : "")} >
          {riskLevelData&& !riskLevelValid ? riskLevelData?.map((asset:any)=><WeatherCard2 name={asset['ITEM_ENG_NM']} ticker={asset['ITEM_CD_DL']} key={asset['ITEM_CD_DL']} riskLevel={asset['CVaR_LV']} maxLoss={asset['CVaRNTS']} price={asset['ADJ_CLOSE']} currency={asset['CURR']} priceChange={""} chartData={JSON.parse(asset['CHART'])} weather={asset['WTHR_ENG_NM']} weatherExplain={asset['WTHR_ENG_DL']}/>)
          : 
